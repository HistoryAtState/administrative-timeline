...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" queryBinding="xslt3">
    ...
    <pattern id="date-alignment-checks">
        ...
        <rule context="tei:date[@notBefore and @notAfter]">
            ...
            <assert role="warning" test="matches(normalize-space(.), '^' || $expected-regex || '$')">Expected “<value-of select="."/>” to be formatted as “<value-of select="$expected-regex"/>”, according to uncertain date range rules.</assert>
        </rule>
    </pattern>
